...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |v|
    ...
    v.customize ["modifyvm", :id, "--vram", "35"]
    v.memory = 7018
    v.cpus = 4
  ...
  config.vm.define "master" do |machine|  
    ...
    #machine.disksize.size = '80GB' #if plugin not installed then run: vagrant plugin install vagrant-disksize
    ...
    machine.vm.provision "shell", inline: <<-SHELL
      ...
      chsh -s /bin/bash vagrant
      #ansible-playbook -i ./inventory/hosts --vault-password-file /opt/ansible/vault_password localE2e.yml
      ansible-playbook localE2e.yml
      source /etc/profile.d/myvars.sh
    SHELL
  end
end
